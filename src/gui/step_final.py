...
class FinalStep(BaseStep):
    ...
    async def show(self, stepper, first_step=False, last_step=False) -> None:
        with ui.step(self.name):
            self.add_help(
                """
- **Review Config**: Review the compiled configuration generated by the wizard.
- **Check Syntax**: Validate INI syntax before persisting.
- **Save Config**: Save configuration to `config.ini` and write cropped reference marker files to disk.
- **Take In Use**: Apply the new configuration directly to the active system runtime.
                """
            )
            with ui.row():
